 ...
 // Expandir ou encolher as mensagens
  ...
  apoiar(evento, item){
    ...
    if(this.config.usuarioApoiouSolicitacao(item))
      ...
      this.config.removerApoio(item.id, solicitante, this.config.getMinhasSolicitacoes());
    else
      // Adicionar o apoio no vetor de solicitações antes da resposta da API
      this.config.adicionarApoio(item.id, solicitante, this.config.getMinhasSolicitacoes());
